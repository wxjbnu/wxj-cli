...
function buildxml(filename){
    ...
    const fileDist = path.join(process.cwd(),'dist','pages',filename.replace(/.wxj/,'')+'.html')
    fs.readFile(filepath, 'utf8', function (err, data) {
        ...
        // 生成最终的xml
        let xmldata = $('temp').html().replace(/temp/g,'template')
        ...
        fs.writeFile(fileDist, xmldata, function (err) {
            if (err) throw err;
